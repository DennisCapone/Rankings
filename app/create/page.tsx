...
export default function Create() {
  ...
  // Function to save the ranking and navigate to the sharing page //
  const Save = async () => {
    ...
    <>
      ...
      {items.map((item, i) => (
        <div key={i} className='mb-3 ml-3 flex justify-center'>
          ...
          <button
          onClick={() => { remove(i) }}
          className='cursor-pointer border-black border-solid border-2 h-14 w-14 text-xl rounded-2xl'
          > X </button>
        </div>
      ))}
      ...
      <div className='flex justify-center'>
        <button 
          onClick={() => add()} 
          className='cursor-pointer rounded-2xl h-14 text-xl border-black border-solid border-2 w-35'
          > AGGIUNGI </button>
      </div>
      <div className='flex justify-center'>
        <button 
