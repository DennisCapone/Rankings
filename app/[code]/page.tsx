...
export default function Play({params} : {params: Promise<{code:string}>}) {
  ...
  useEffect(() => {giveQuestion()}, [])
  return (
    <>
      <Link href={`/${code}/ranking`}><div className='ml-5 mt-5'><Button textcolor="" bcolor="" text="Classifica" color="bg-green-300" /></div></Link>
      <div className='flex justify-center mt-5 gap-10 mt-50'><button onClick={async () => {await eloSystem(true); await giveQuestion()}}><Button textcolor="" color="" bcolor="" text={textOne} /></button><button onClick={async () => {await eloSystem(false); giveQuestion()}}><Button text={textTwo} textcolor="" bcolor="" color="" /></button></div>
