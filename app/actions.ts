...
export async function addRanking(items:string[], name:string) {
  await db.ranking.create({
    if (items.length < 2) {throw new Error("Invalid input")}                // Check error //
    let code = ""
    for (let ind = 0; ind < 8; ind++) code += String.fromCharCode(Math.floor(Math.random() * (90 - 65 + 1) + 65))    /** make the code and create a ranking with
